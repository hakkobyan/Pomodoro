...
const GradientBlinds = ({
  ...
  const staticPointerRef = useRef(false);
...
`;
    ...
    const setPointerToFixedPosition = () => {
      const x = gl.drawingBufferWidth * 0.5;
      const y = gl.drawingBufferHeight * 0.5;
    ...
    const updatePointerMode = () => {
      ...
      staticPointerRef.current = window.matchMedia(
        "(max-width: 1024px), (hover: none), (pointer: coarse)"
      ).matches;

      if (staticPointerRef.current) {
        uniforms.uSpotlightRadius.value = Math.max(spotlightRadius, 1.05);
        setPointerToFixedPosition();
      } else {
    ...
    const onPointerMove = (e) => {
      if (staticPointerRef.current) return;
      const rect = container.getBoundingClientRect();
